...
PACKET_FIXED_OVERHEAD = 7  # packet_type, message_id, fragment_idx, total_fragments, payload_len, crc_value

# Tamanho máximo do payload que podemos colocar em nosso Packet: VW_MAX_PAYLOAD (27) - PACKET_FIXED_OVERHEAD (6) = 21 bytes
MAX_PACKET_PAYLOAD_SIZE = (27 - PACKET_FIXED_OVERHEAD)  # Resulta em 21 bytes
...
# < : little-endian (Arduino é little-endian na maioria dos casos para tipos simples)
# B : unsigned char (uint8_t)
# 21s: 21 bytes string (para o payload_data)
PACKET_FORMAT = f"<BBBHB{MAX_PACKET_PAYLOAD_SIZE}sB"

# Tabela de lookup para CRC-4 (polinômio G(x) = x^4 + x + 1, ou 0x3 com x^4 implícito)
# Esta tabela deve ser EXATAMENTE igual à do Arduino (pgm_read_byte)
CRC4_TABLE = (
    0x0, 0x3, 0x6, 0x5, 0xC, 0xF, 0xA, 0x9, 0xB, 0x8, 0xD, 0xE, 0x7, 0x4, 0x1, 0x2
)


def calculateCRC4(data: bytes) -> int:
    """
    Calcula o CRC-4 de uma sequência de bytes usando o mesmo algoritmo do Arduino.
    data: bytes - os bytes sobre os quais o CRC será calculado.
    Retorna: int - o valor do CRC-4 (0-15).
    """
    crc = 0x00
    for byte in data:
...
class ArduinoController:
                    ...
                    if self.ser.in_waiting >= TOTAL_PACKET_SIZE:
                        ...
                        try:
                            ...
                            if packet_type == PACKET_TYPE_DATA:
                                ...
                                calculated_crc = calculateCRC4(data_for_crc)
    ...
    def _send_packet(self, packet_type: int, message_id: int, fragment_idx: int, total_fragments: int,
        ...
        crc_value = calculateCRC4(data_for_crc)
